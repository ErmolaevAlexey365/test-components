...
interface ICardImageButtonProps {
    data: any
    setAdditionalSkills: (data: any) => void
    evaluation: number
}

const AdditionalSkills: React.FC<ICardImageButtonProps> = ({data, setAdditionalSkills, evaluation}) => {
    ...
    const selectToggle = (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        let updateEvaluation = (data.skills.filter((el: any) => el.id == value)).map((el: any) => (
            {...el, evaluation: evaluation}
        ));

        setAdditionalSkills(updateEvaluation[0])
