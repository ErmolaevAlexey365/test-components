...
import AdditionalAbilities from '@/components/common/additionalAbilities';
...
const ProfilingAbilitiesListContent: FC<IProfilingComponentProps> = ({answerQuestion, revertAnswer}) => {
    ...
    const [additionalAbilities, setAdditionalAbilities] = useState<any>()
    ...
    const submitHandler = async (e: any) => {
        e.preventDefault()
        let resultAbilities: any = []
        elements.forEach((el: any) => {
            let objAbility: any = {};
            objAbility["abilityId"] = el.ability.id
            objAbility["evaluation"] = el.evaluation
            resultAbilities.push(objAbility)
        })

        await answerQuestion({
            answer: {
                type: "PSAbilitiesConfirm",
                questionIdentifier: "PSAbilitiesConfirm",
                resultAbilities: resultAbilities
            }
        })
    }
    ...
    useEffect(() => {
        userProfProfilingService.getCurrentQuestion().then(res => {
            ...
            let newArray = (res.question as any).abilities.filter((el: any) => el.evaluation >= 3)
    ...
    useEffect(() => {
        ...
        setElements([
            ...elements,
            {
                ability: additionalAbilities,
                evaluation: additionalAbilities.evaluation
            }
        ])
        ...
        <>
                ...
                <div className="container">
                    <div className="flex flex-col">
                        <p className="font-medium text-[#282421] mb-[8px] mt-[20px]">
                            הנה רשימת היכולות החזקות שלך
                        </p>
                        <p className="font-normal text-[#282421] mb-[20px]">
                            על בסיס ההגדרות שלך, הכנו את רשימת היכולות שבהן אתה ממש טוב.שים לב &#x2665; שווה לקחת רגע כדי לדייק את הרשימה. למה? כי
                            היא תשמש לזיהוי ההזדמנויות שנציג לך בסוף התהליך.אפשר להסיר ולהוסיף יכולות.
                        </p>
                                        ...
                                        {(provided) => (
                                            <div {...provided.droppableProps} ref={provided.innerRef}>
                                                {elementsConverted["4"]?.map((item: any, index: any) => (
                                                    <Draggable key={item.ability.original_name_he} draggableId={item.ability.original_name_he}
                                                            index={index}>
                                                        {(provided) => {
                                                            return (
                                                                <div
                                                                ...
                                                                >
                                                                    ...
                                                                    <SkillAbilityCardComponent key={index}
                                                                                            ...
                                                                                            title={item.ability.original_name_he}/>
                                                                </div>
                                                            );
                                                        }}
                                                    </Draggable>
            ...
            <ModalLayoutFullScreen open={modalAwesome} setClose={() => setModalAwesome(false)} title="">
                <div dir='rtl' className="flex flex-col w-full">
                    ...
                    <div className="flex flex-col">
                        {
                            currQuest?.additionalAbilities.map((el: any, index: number) =>
                                <AdditionalAbilities key={index} data={el} setAdditionalAbilities={setAdditionalAbilities} evaluation={4}/>
                            )
                        }
                    </div>
                </div>
            ...
            <ModalLayoutFullScreen open={modalGood} setClose={() => setModalGood(false)} title="">
                <div dir='rtl' className="flex flex-col w-full">
                    ...
                    <div className="flex flex-col">
                        {
                            currQuest?.additionalAbilities.map((el: any, index: number) =>
                                <AdditionalAbilities key={index} data={el} setAdditionalAbilities={setAdditionalAbilities} evaluation={3}/>
                            )
                        }
                    </div>
                </div>
...
export default ProfilingAbilitiesListContent;