...
const ProfilingAbilitiesContent: FC<IProfilingComponentProps> = ({answerQuestion, revertAnswer, currentQuestion}) => {
    const [modal, setModal] = useState<boolean>(false)
    const [evaluationValue, setEvaluationValue] = useState<number | null>(null)
    const sendDataHandler = async () => {
        answerQuestion({
            answer: {
                type: "PSEvaluateAbility",
                questionIdentifier: "PSEvaluateAbility",
                ability: {
                    abilityId: currentQuestion.ability.id,
    ...
    const submitHandler = async (e: any) => {
        e.preventDefault()
        revertAnswer()
        // await professionalProfilingService.profilingRevertAnswer().then(res => {
        //     // setCurrentQuestionType(res.data.question.type)
        //     setQuest({
        //         question: res.data.question?.ability?.original_name_he,
        //         total: res.data.question?.abilitiesCount,
        //         order: res.data.question?.abilityPosition
        //     })
        // })
    }
                        ...
                        <div className="hidden lg:flex justify-between mt-[80px]">
                            <div className="flex">
                                <p className="font-medium font-semibold text-[#282421] ml-[16px]">
                                    {currentQuestion.ability.original_name_he}
                                </p>
                            ...
                            <p className="font-medium text-[#282421] mb-[20px]">
                                {currentQuestion.abilityPosition}/{currentQuestion.abilitiesCount}
                            </p>
                        </div>
            ...
            <ModalLayout open={modal} setClose={() => setModal(false)}>
                <div dir="rtl" className="flex flex-col justify-center items-center ">
                    <p className="lg:top-[26px] lg:right-[26px] text-lg text-[#143E29] mb-[8px]">
                        {currentQuestion?.ability && currentQuestion?.ability.original_name_he}
                    </p>
                    <p className="text-semimd font-normal mb-[28px]">
                        מה זה אומר?
                    </p>
                    <p className="text-semimd font-normal text-right mb-[28px]">
                        {currentQuestion?.ability && currentQuestion?.ability.original_desc_he}
                    </p>
...
export default ProfilingAbilitiesContent;