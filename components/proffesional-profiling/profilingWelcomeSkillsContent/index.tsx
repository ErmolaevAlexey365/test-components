...
const
    ProfilingWelcomeSkillsContent: React.FC<IProfilingComponentProps> = ({answerQuestion, revertAnswer}) => {

        const submitHandler = async (e: any) => {
            e.preventDefault()
            answerQuestion({
                answer: {
                    type: "PSSkillsWelcome",
                    questionIdentifier: "PSSkillsWelcome"
                }
            })
        }
                ...
                <div className="container relative px-[12px] py-[20px] shadow-[0px_0px_50px_rgba(40,36,33,0.12);] lg:shadow-none">
                    <div className="flex flex-col items-center justify-between min-h-[536px]">
                        <div className="text-semimd">
                            <p>
                                או.קיי, על סמך כל התפקידים שמילאת, נציג לך עכשיו רשימה של כישורים אפשריים. שוב, נבקש ממך לעבור עליהם, לחשוב
                                "עד כמה אני חזק בזה" ואז לדרג בהתאם.
                            </p>
                        </div>
                        <Image
                            src={lines}
...
export default ProfilingWelcomeSkillsContent;